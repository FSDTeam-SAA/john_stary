...
export function InteractiveHotspot({ position, tooltip, onClick, className, delay = 0 }: InteractiveHotspotProps) {
  ...
  return (
    <button
      ref={hotspotRef}
      className={cn(
        "absolute z-10 cursor-pointer bg-white/30 backdrop-blur-md border-2 border-white/50 rounded-full hover:bg-white/50 transition-colors duration-300 flex items-center justify-center",
        className,
      )}
      style={position}
      onClick={onClick}
