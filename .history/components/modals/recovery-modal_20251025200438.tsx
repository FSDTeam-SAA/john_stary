...
export function RecoveryModal({ isOpen, onClose }: RecoveryModalProps) {
  ...
  return (
    ...
    >
      <div
        ref={contentRef}
        className="relative w-full max-w-6xl aspect-video overflow-hidden rounded-xl bg-black shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <Button
          variant="ghost"
          size="icon"
          className="absolute right-4 top-4 z-10 rounded-sm bg-amber-500 px-8 py-2 text-sm font-bold text-black hover:bg-amber-600"
          onClick={onClose}
        >
          CLOSE
        </Button>
        ...
        <Image
          fill
          src="/assets/images/RECOVERY PAGE.png"
